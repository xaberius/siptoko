...
SQL = "Select KodeSales as [Kode Sales],namaSales as [Nama Sales], " & _
    "Alamat,Kota,Telepon from Sales order by KodeSales"
...
End If
    ...
    For Each Query In Kata()
        Kalimat = Kalimat + " or namaSales like '%" & Query & "%'"
    Next Query
    Kalimat = Mid(Kalimat, 4, Len(Kalimat) - 3)

SQL = "select KodeSales as [Kode Sales],NamaSales as [Nama Sales],Alamat,Kota,Telepon " & _
    " from Sales where " & Kalimat & _
    " order by KodeSales"
Set Grid.DataSource = DbCon.Execute(SQL)
Grid.Refresh
TxtCari = ""
...
If Edit Then
    SQL = "update Sales set namaSales='" & Trim(TxtNama) & "',alamat='" & _
    Trim(TxtAlamat.Text) & "',kota='" & Trim(TxtKota.Text) & "',telepon='" & _
    Trim(TxtTelepon.Text) & "' where kodeSales='" & Trim(TxtKode.Text) & "'"
    DbCon.Execute SQL
    MsgBox "Data Saved"
    Form_Load
ElseIf Not Edit Then
    TxtKode = KodeAuto
    SQL = "Insert Into Sales values ('" & Trim(TxtKode.Text) & "','" & Trim(TxtNama.Text) & _
...
If KeyCode = 46 Then
    If MsgBox("Sales " & Trim(Grid.Columns(1).Text) & " Yakin Akan Dihapus", vbYesNo + vbCritical) = vbYes Then
        SQL = "delete from Sales where kodeSales='" & Trim(Grid.Columns(0).Text) & "'"
        DbCon.Execute SQL
        MsgBox "Data Deleted"
        Form_Load
    End If
...
'   Exit Function
'End If
SQL = "Select KodeSales from Sales order by KodeSales Desc"
Set RSFind = DbCon.Execute(SQL)
If RSFind.BOF Then
    KodeAuto = "0001"
Else
    KodeAuto = Format(CInt(Left(RSFind!KodeSales, 4)) + 1, "0000")
End If
End Function
